...
export class UIManager {
  ...
  updateExpenseTable(expenses) {
    ...
    expenses.forEach(expense => {
      ...
      rowsMobile += `
        <tr class="hover:bg-zinc-50 dark:hover:bg-slate-700 transition-colors duration-150" tabindex="0">
          <td class="px-16 py-12 whitespace-nowrap text-sm text-text-primary dark:text-white">${formatDate(expense.tanggal)}</td>
          <td class="px-16 py-12 whitespace-nowrap">
            <span class="inline-flex items-center px-8 py-4 text-xs font-medium rounded-md border bg-zinc-100 dark:bg-slate-700 text-zinc-700 dark:text-slate-300 border-zinc-200 dark:border-slate-600">
              ${capitalizeCategory(expense.kategori)}
            </span>
          </td>
          <td class="px-16 py-12 text-sm text-text-secondary dark:text-slate-400 hidden md:table-cell max-w-xs truncate">${expense.catatan || '-'}</td>
          <td class="px-16 py-12 whitespace-nowrap text-right text-sm font-semibold text-text-primary dark:text-white">${formatRupiah(expense.nominal)}</td>
          <td class="px-16 py-12 whitespace-nowrap text-right text-sm">
            <div class="flex justify-end gap-6">
              <button onclick="editPengeluaran('${expense.id}')"
                class="text-text-secondary dark:text-slate-400 hover:bg-zinc-100 dark:hover:bg-slate-700 p-6 rounded-lg transition-all duration-150"
              ...
              <button onclick="hapusPengeluaran('${expense.id}')"
                class="text-destructive hover:bg-red-50 dark:hover:bg-red-900/30 p-6 rounded-lg transition-all duration-150"
      ...
      rowsDesktop += `
        <tr class="hover:bg-zinc-50 dark:hover:bg-slate-700 transition-colors duration-150" tabindex="0">
          ...
          <td class="px-16 py-12 text-sm text-text-secondary dark:text-slate-400 max-w-xs truncate">${expense.catatan || '-'}</td>
          <td class="px-16 py-12 whitespace-nowrap text-right text-sm font-semibold text-text-primary dark:text-white">${formatRupiah(expense.nominal)}</td>
          <td class="px-16 py-12 whitespace-nowrap text-right text-sm">
            <div class="flex justify-end gap-6">
              <button onclick="editPengeluaran('${expense.id}')"
